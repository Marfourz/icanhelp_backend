...
from api.utils.errors import ErrorCode, validation_error
...
class DiscussionViewSet(
    ...
    def get_queryset(self):
        ...
        ).filter(users__id=user_profil.id).order_by('-updatedAt')

        for d in discussions:
            since = d.lastOpenAt or epoch
            d.nbMessagesNotRead = d.messages.filter(
                createdAt__gt=since
            ).exclude(sender=user_profil).count()

        return discussions

    @action(detail=True, methods=['get'])
    def messages(self, request, pk=None):
        """Récupérer les messages d'une discussion et marquer comme lu."""
        user_profil = self.get_user_profil()
        ...
        UserDiscussionMetaData.objects.update_or_create(
            discussion=discussion,
            user=user_profil,
            defaults={'lastOpenDiscussionAt': timezone.now()},
        )
